...
class MIRASystem:
    ...
    def get_mira_outputs(self, vitals_tensor, times_tensor, steps=5):
        ...
        with torch.no_grad():
            # 3. Get Hidden States (The Neural Latent Space)
            outputs = self.model(
                input_ids=v_reshaped,
                time_values=t_expanded,
            ...
            cur_times = t_expanded.clone()
            preds_norm = []

            for _ in range(steps):
                f_out = self.model(input_ids=cur_vals, time_values=cur_times)
                next_val = f_out.logits[:, -1, :] # [5, 1]
                preds_norm.append(next_val.unsqueeze(0)) # Store for output

                # Update window for next hour
                cur_vals = torch.cat([cur_vals, next_val.unsqueeze(1)], dim=1)
                next_t = cur_times[:, -1:] + 1.0
                cur_times = torch.cat([cur_times, next_t], dim=1)

            # 5. Inverse-Normalize Forecasts back to Clinical Units
            # Shape: [steps, 5] -> [1, steps, 5]
            preds_combined = torch.cat(preds_norm, dim=0).permute(2, 0, 1) 
