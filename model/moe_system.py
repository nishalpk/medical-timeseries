...
class FullSystemMoE(nn.Module):
    def __init__(self, time_features=1, time_latent_dim=128, text_embed_dim=3072):
        super().__init__()
        self.time_series_encoder = MIRATimeSeriesEncoder(input_features=time_features, hidden_dim=time_latent_dim)
    ...
    def forward(self, raw_time_series, graphrag_embeddings):
        mira_latent = self.time_series_encoder(raw_time_series)
        weights = self.llm_router(graphrag_embeddings) 
        experts_out = torch.stack([self.hemodynamic_expert(mira_latent), 
                                   self.biochemical_expert(mira_latent), 
                                   self.generalist_expert(mira_latent)], dim=1)
